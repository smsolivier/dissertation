...
\caption{A second-order curved mesh and an example direction $\Omegahat$. The element $K_1$ has a concave face that is shared with element $K_3$. For the direction shown by $\Omegahat$, $K_1$ is upwind of itself. This means that $\Omegahat\cdot\n$ changes sign along the face $\mathcal{F} = K_1 \cap K_3$. }
