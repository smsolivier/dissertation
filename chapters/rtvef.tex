...
Since $\mat{M}$ is a mass matrix, it is symmetric positive definite and thus, $\ubar{v}^T \mat{M}\ubar{q} =0 \iff \ubar{q} = 0$. In other words, we have shown that $f = 0 \Rightarrow \ubar{q} = 0$. Setting $\ubar{q} = 0$ in the first row of Eq.~\ref{rtvef:block_sys} yields: 
	\begin{equation}
		\mat{D}^T \ubar{u} = 0 \,. 
	\end{equation}
For the block system to be non-singular, we must have that $\mat{D}^T \ubar{u} = 0 \iff \ubar{u} = 0$. In other words, $N(\mat{D}^T) = 0$. The discrete inf-sup condition is precisely this condition on the matrix $\mat{D}$ that $N(\mat{D}^T) = 0$. 
