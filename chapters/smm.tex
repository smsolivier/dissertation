...
\section{VEF and SMM as Newton Methods}
Newton's method solves the root-finding problem $F(\y) = 0$ by linearizing $F$ about the previous iteration. The iterative scheme is then 
	\begin{equation}
		0 = F(\y) \xrightarrow{\text{TSE}} F(\y_n) + \pderiv{F}{\y}\biggr\rvert_{\y_n}(\y_{n+1} - \y_n) \Rightarrow \y_{n+1} = \y_n - \bracket{\pderiv{F}{\y}\biggr\rvert_{\y_n}}^{-1} F(\y_n) \,. 
	\end{equation}
...
From Section \ref{dgvef_sec:ip}, the IP VEF discretization is: find $\varphi\in Y_p$ such that 
	...
	\end{multline}
where $\kappa$ is the penalty parameter. Evaluating this discretization when the angular flux is linearly anisotropic gives
	\begin{equation}
...
The IP SMM discretization is then: find $\varphi \in Y_p$ such that 
	\begin{multline}
		\frac{1}{2}\int_{\Gamma_b} u\varphi \ud s + \int_{\Gamma_0} \kappa\, \jump{u}\jump{\varphi} \ud s - \int_{\Gamma_0} \jump{u}\avg{D\nablah\varphi\cdot\n} \ud s - \int_{\Gamma_0} \avg{D\nablah u\cdot\n} \jump{\varphi} \ud s \\
		+ \int \nablah u \cdot D\nablah\varphi \ud \x + \int \sigma_a\, u \varphi \ud \x \\
		= \int u\, Q_0 \ud \x + \int \nablah u \cdot \frac{\vec{Q}_1}{\sigma_t} \ud \x - \int_{\Gamma_0} \jump{u} \avg{\frac{\vec{Q}_1\cdot\n}{\sigma_t}} \ud s - \int_{\Gamma_b} u\paren{2\Jin + \beta(\psi)} \ud s \\
...
As before with the Eddington tensor, the correction tensor is generally discontinuous across interior mesh interfaces since we assume DG is used for the transport discretization. The CG discretization is then: find $\varphi \in V_p$ such that 
	\begin{multline}
		...
		= \int u\, Q_0 \ud \x + \int \nabla u \cdot \frac{\vec{Q}_1}{\sigma_t} \ud \x - \int_{\Gamma_b} u\paren{2\Jin + \beta(\psi)} \ud s \\
...
From Eq.~\ref{rtvef:rtvef}, a Raviart Thomas discretization of VEF is: find $(\varphi,\vec{J}) \in Y_p \times \RT_p$ such that
	...
	\end{multline}
	\end{subequations}
The corresponding diffusion discretization is found by setting $\E = \frac{1}{3}\I$ and $E_b = E_{b0} = E_b(\psi_0)$: 
...
with angular quadrature since it was found to be important that $\Jin$ and $\int |\Omegahat\cdot\n| \ud \Omega$ are integrated with the same numerical quadrature rule. This is because both $\int |\Omegahat\cdot\n|\ud \Omega$ and
	...
	\end{equation}
where $\mathbbm{1}_A(x)$ is an indicator function that is one when $x\in A$ and zero otherwise have non-smooth integrands and therefore cannot be integrated exactly with an angular quadrature rule. 

The correction terms are found by computing the partial derivative of the VEF discretization with respect to the angular flux evaluated at the linearly anisotropic function $\y_0 = \vector{\psi_0 & \varphi_0}$. As before, terms without VEF data are zero and we can replace $E_b\varphi \rightarrow \beta(\psi)$ and $\E\varphi \rightarrow \T(\psi)$. However, this replacement is valid only for terms with \emph{only} $E_b\varphi$ or $\E\varphi$ and not both. An additional linearization is required for the boundary terms due to the presence of both the Eddington tensor and boundary factor. 

The linearization of the boundary term is: 
	\begin{equation}
	\begin{aligned}
	...
	\end{aligned}
	\end{equation}
Since we have assumed that $\varphi_0$ satisfies the boundary conditions, we can subsitute 
	\begin{equation}
...
to arrive at 
	...
	\end{aligned}
	\end{equation}
where we have used that $\pderiv{\E}{\psi}\rvert_{\psi_0}\varphi_0 = \T$, $\pderiv{E_b}{\psi}\rvert_{\psi_0}\varphi_0 = \beta$, and $\E_0 = \frac{1}{3}\I$. 
...
\section{Results}
The methods presented in this section were implemented in the MFEM finite element framework \cite{mfem-paper}. We use the conjugate gradient and MINRES solvers from MFEM along with \emph{hypre}'s \cite{hypre} BoomerAMG solver. KINSOL, from the Sundials \cite{hindmarsh2005sundials} package, is used to solve the fixed-point problems with fixed-point iteration and Anderson-accelerated fixed-point iteration. The high-order DG \gls{sn} solver from \cite{graph_sweeps} was used. Unless otherwise noted, the angular flux and scalar flux are approximated with equal degree finite element spaces. We refer to the four methods derived in this section as the interior penalty (IP), continuous finite element (CG), Raviart Thomas (RT), and hybridized Raviart Thomas (HRT) methods. 
