...
with Dirichlet boundary conditions. The present goal is to adapt their derivation to the VEF equations: 
	...
	\end{equation}
	\end{subequations}
with the Robin style boundary conditions given in Eq.~\todo{ref}. We will see significant differences in the final equation since the Eddington tensor is inside the divergence.
...
We now present operators $\mat{Q} : Y_p \rightarrow V_C$ and $\mat{B} : Y_p \rightarrow V_B$ that take functions in $Y_p$ and convert them to $V_C$ and $V_B$, respectively. Let $u \in Y_p$ be given. The interpolation operator $\mat{Q}u$ copies the degrees of freedom corresponding to nodes interior to each element or on the boundary of the domain (e.g.~$\x_i \in \mathcal{I}(K)$ for each $K$ or $\x_i \in \partial\D$) so that $\mat{Q}u(\x_i) = u(\x_i)$ at each $\x_i \in \mathcal{I}(K)$ for each $K$. For nodes located on interior faces of the mesh, $\mat{Q}u$ averages the values of $u$ in the elements that share the face. For each $\mathcal{F} \in \Gamma_0$ we define the set $\mathcal{S}_\mathcal{F} = \{ K \in \meshT : K \cap \mathcal{F} \neq \emptyset\}$ as the collection of elements that share the face $\mathcal{F}$. At a node $\x_i \in \mathcal{F}$, where $\mathcal{F} \in \Gamma_0$, we set 
	\begin{equation}
		\mat{Q}u(\x_i) = \frac{1}{\mathrm{card}(\mathcal{S}_\mathcal{F})} \sum_{K \in \mathcal{S}_\mathcal{F}} u|_K(\x_i) \,, 
	\end{equation}
where $\mathrm{card}(\mathcal{S}_\mathcal{F})$ denotes the size of the set $\mathcal{S}_\mathcal{F}$ (i.e.~the number of elements that share the face $\mathcal{F}$). 
