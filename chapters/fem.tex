...
This can be seen by setting the gradient of $\Pi$ to zero: 
	...
	\end{equation}
The conjugate gradient method \cite{cg_hs,cgh} is built around the idea of minimizing $\Pi$ in such a way that the system is guaranteed to be solved in at most $N$ steps, where $N$ is the number of rows or columns in the square matrix $\mat{A}$. 
