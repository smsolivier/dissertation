...
Both the mesh and the solution are represented with a polynomial space defined locally on each element. Let $\hat{K}^{\dim}$ denote the reference element which we set to the unit $\dim$-dimensional cube, $\hat{K} = [0,1]^{\dim}$. Where the dimension of the reference element is obvious, we omit the superscript. We define the space of univariate polynomials of degree less than or equal to $k$ as: 
	\begin{equation}
		\mathcal{P}_k(\hat{K}^1) = \{ p : \hat{K} \rightarrow \R : p = \sum_{i=0}^{k} \alpha_i x^i \,, \alpha_i \in \R\ \forall\,i \in [0,k]\} = \spn\{ 1, x, x^2, \ldots, x^k \} \,. 
	\end{equation}
...
We then have that the characteristic length of the face is 
	\begin{equation}
		h_\mathcal{F} = \paren{\int_{\hat{K}^{\dim-1}} J_\mathcal{F}\ud s}^{1/(\dim-1)} \,. 
	\end{equation}
The normal vector to $\mathcal{F}$ can be be computed by leveraging the fact that the gradient of the embedded transformation points \emph{tangent} to the face. The normal is then computed by rotating the tangent vector. Since $\F_\mathcal{F}$ is the gradient of $\T_\mathcal{F}$, its columns represent vectors that are tangent to $\mathcal{F}$. The normal direction is then found by applying a suitable rotation to one of the columns of $\F_\mathcal{F}$. In two dimensions, $\F_\mathcal{F}$ must be rotated by $-90^\circ$. Thus, the normal vector is 
	\begin{equation}
		\n = \frac{\mat{R}\F_\mathcal{F}}{\|\mat{R}\F_\mathcal{F}\|}
	\end{equation}
where $\mat{R}$ is the rotation matrix 
	\begin{equation}
		\mat{R} = \begin{bmatrix} 
			\cos(\theta) & -\sin(\theta) \\ \sin(\theta) & \cos(\theta) 
		\end{bmatrix}
	\end{equation}
with $\theta = -\pi/2$. The direction of the tangent vector is dependent on the orientation of the embedded surface. Thus, it is important to use a consistent orientation of the surfaces so that the rotation process always produces the outward normal. Due to its close relation to the gradient of the embedded transformation, the normal vector will vary as the gradient of the space used for the volumetric transformation. In other words, a mesh with linear faces will have constant normal vectors, quadratic faces will have linear normal vectors, etc. 
% --- quadratic line normal diagram --- 
\begin{figure}
\centering
\includegraphics[width=.5\textwidth]{figs/normal.pdf}
\caption{The tangent and normal vectors for the face between the two elements of the mesh depicted in Fig.~\ref{fem:quadmesh}. The transformation for the embedded surface between the two elements is defined by the quadratic polynomial in the space $\mathcal{Q}_2(\hat{K}^1)$ that interpolates between the nodes labeled 2, 7, and 12. The tangent vectors are computed from the Jacobian matrix of the embedded transformation. The normals are computed by rotating the tangent vectors by $-90^\circ$.}
...
Let $\mat{N} = \vector{\ell_1 & \ldots & \ell_4}$ and $\mat{X}$ the matrix of nodal positions corresponding to the nodal basis such that 
	\begin{equation}
		...
		\end{bmatrix} \,. 
	\end{equation}
The transformation $\T = \sum_i \x_i \ell_i$ can be written as 
	\begin{equation}
		\T = \mat{X} \mat{N} \,. 
	\end{equation}
...
We conclude with a computation of the embedded transformation associated with the right face of the transformation described by $\T$. This face corresponds to fixing $\xi = 1$ and letting $\eta \in [0,1]$ vary. Let 
	...
	\end{equation}
be the nodal basis for $\mathcal{Q}([0,1])$ where $\chi \in [0,1]$ denotes the reference coordinate for $\mathcal{F}$. The embedded transformation is then
	\begin{equation}
...
\subsection{\todo{Examples}}

\section{\todo{Iterative Solution Methods for Linear Systems}}
\end{document}