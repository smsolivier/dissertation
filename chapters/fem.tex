...
We then have that the characteristic length of the face is 
	...
	\end{equation}
The normal vector to the face $\mathcal{F}$ can be computed with Nanson's formula \cite{oden1971finite,ciarlet_elasticity}: 
	\begin{equation}
		\n \ud s = J \mat{F}^{-T} \hat{\n} \ud \hat{s} \,,
	\end{equation}
where $\mat{F}$ and $J$ are the volumetric Jacobian matrix and determinant evaluated on the face $\mathcal{F}$ and $\hat{\n}$ is the normal vector in reference space. Such a formula is advantageous when only the volumetric transformations are accessible. The normal direction can be also be computed by leveraging the fact that the gradient of the embedded transformation points \emph{tangent} to the face. The normal is then computed by rotating the tangent vector. Since $\F_\mathcal{F}$ is the gradient of $\T_\mathcal{F}$, its columns represent vectors that are tangent to $\mathcal{F}$. \todo{how to rotate to get normal}
...
By example, the transformation, Jacobian matrix, and determinant for the transformation depicted in Fig.~\ref{fem:trap_trans} are 
	\begin{equation}
		...
		J = 2 \alpha \eta h + h^2 \,. 
	\end{equation}
For example, the inverse of the transformation shown in Fig.~\ref{fem:trap_trans} is 
	\begin{equation}
		\T^{-1}(\x) = \begin{bmatrix} 
