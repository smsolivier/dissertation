...
\todo{non-conforming, minimization of potential, alternative variational forms}
...
We then have that the characteristic length of the face is 
	...
	\end{equation}
The normal vector to $\mathcal{F}$ can be be computed by leveraging the fact that the gradient of the embedded transformation points \emph{tangent} to the face. The normal is then computed by rotating the tangent vector, $\tang$, such that $\tang\cdot\n = 0$. Since $\F_\mathcal{F}$ is the gradient of $\T_\mathcal{F}$, its columns represent vectors that are tangent to $\mathcal{F}$. The normal direction is then found by applying a suitable rotation to one of the columns of $\F_\mathcal{F}$. In two dimensions, $\F_\mathcal{F}$ is a $2\times 1$ matrix and thus only has one column which corresponds to the direction tangential to $\mathcal{F}$. The normal direction is found by rotating the tangent direction by $-90^\circ$. This process is depicted for the face between the two elements of Fig.~\ref{fem:quadmesh} in Fig.~\ref{fem:normal_diag}. 

In other words, the normal vector is computed with:
	\begin{equation}
		\n = \frac{\mat{R}\F_\mathcal{F}}{\|\mat{R}\F_\mathcal{F}\|}
	\end{equation}
where $\mat{R}$ is a rotation matrix \todo{look at MFEM code}. In two dimensions, we use 
	\begin{equation}
		\mat{R} = \begin{bmatrix} 
