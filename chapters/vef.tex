...
Let, 
	...
	\end{equation}
	\begin{equation}
		\mat{R}(\psi)\varphi = -\nabla\cdot\frac{1}{\sigma_t}\nabla\cdot\paren{\E\varphi} + \sigma_a \varphi 
	\end{equation}
...
where 
	...
	\end{equation}
The fixed-point operator $G$ is applied in two stages: 1) solve the transport equation using a scattering source formed from the VEF scalar flux or an initial guess and 2) solve the VEF drift-diffusion equation using the VEF data computed from the angular flux from stage 1). Note that by eliminating the angular flux, it only appears as an auxiliary variable used to compute the VEF data. This iteration is depicted in Fig.~\ref{vef:vef_alg}. 
...
In the thick diffusion limit, the angular flux is a linearly anisotropic function in angle. In other words, for some spatially-dependent functions $f(\x)$ and $\vec{g}(\x)$, 
	\begin{equation}
